...
export function TerminalView({ visible }: { visible: boolean }) {
  ...
  const { tabs, activeId, ready, newTab, setTitle, focusTab, closeTab } = useTabs();
      ...
      <div ref={paneArea} className="relative min-h-0 flex-1">
        {tabs.map((tab) => {
          ...
          return <div key={tab.termId} className={`absolute overflow-hidden ${rects.length > 1 ? `border ${tab.termId === activeId ? "border-edge3" : "border-edge"}` : ""}`}
            ...
            <div className="min-h-0 flex-1 px-4 py-3"><TerminalPane termId={tab.termId} active={shown} focused={shown && tab.termId === activeId} onTitle={(title) => setTitle(tab.termId, title)} /></div>
          </div>;
        })}
