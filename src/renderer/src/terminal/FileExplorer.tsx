...
export function FileExplorer({ cwd: activeCwd, onClose }: { cwd?: string; onClose: () => void }) {
  ...
  return <section className="flex w-[340px] shrink-0 flex-col border-l border-edge bg-panel font-sans">
    ...
    {!cwd ? <div className="p-3 text-xs text-dim">Open a terminal to browse its project.</div> : selected && original ? <>
      ...
      {editing ? <textarea aria-label={`Edit ${selected}`} spellCheck={false} value={text} onChange={(event) => setText(event.target.value)} onKeyDown={(event) => { if ((event.metaKey || event.ctrlKey) && event.key === "s") { event.preventDefault(); void save(); } }} className="min-h-0 flex-1 resize-none bg-bg p-3 font-mono text-[11px] leading-5 text-soft outline-none" /> : <div className="min-h-0 flex-1 select-text overflow-auto p-3">{/\.md$/i.test(selected) ? <Markdown>{text}</Markdown> : <pre className="font-mono text-[11px] leading-5 text-soft">{text}</pre>}</div>}
    </> : <>
